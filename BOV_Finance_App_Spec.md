...
- Produces an A4 landscape PDF: dated header, transaction table (Date / Description / Account / Category / Amount / Receipt), total row
...
- Generated entirely in the browser using jsPDF + autoTable; no Python required

### 5.5 FL3XX Report

Two-part output generated for a selected month:

**Part 1 — Transaction table PDF (browser):** generated from the Reports tab, same format as the Business report (including Prepare descriptions step) but filtered to FL3XX = Y transactions. PDF title: **FL3XX Expenses Report**. Downloaded immediately.

**Part 2 — Combined PDF with receipts (Python):** run `supabase/generate_fl3xx_report.py` from Terminal. The script:
- Fetches FL3XX transactions for the selected month from Supabase
- Downloads each linked receipt from Google Drive (handles both PDFs and images; images are converted to PDF automatically)
- Merges the cover page (transaction table, generated by fpdf2) with receipt pages (merged by pypdf) in date order
- Saves `FL3XX_Report_YYYY-MM.pdf` to the Bank folder

Google Drive access for the Python script uses a separate **Desktop app** OAuth client (created in the same Google Cloud project). The token is stored locally after first run — no re-authentication needed on subsequent months.
